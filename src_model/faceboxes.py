...
from pytorch_to_caffe import neg
...
class CRelu(nn.Module):
    ...
    def forward(self, x):
        ...
        y = neg(x)
        return F.relu(torch.cat([x, y], 1), inplace=True)
...
if __name__ == '__main__':
    our_face_boxes_net = FaceBoxes(phase='train', size=1024, num_classes=2)
    image = torch.zeros([2, 3, 1024, 1024])
    output_feature = our_face_boxes_net(image)
